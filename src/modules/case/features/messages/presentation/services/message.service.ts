...
export class MessageService {
  constructor(
    ...
    private readonly notifier = new NotificationService(new NotificationRepository())
  ) {}
  ...
  async send(caseId: number, dto: SendMessageDTO, user: CurrentUser) {
    ...
    if (!lawyerId) {
      throw new AppError("Un abogado debe estar asignado", HttpStatusCodes.CONFLICT.code);
    }
